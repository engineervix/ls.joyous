...
class BorgPageForm(WagtailAdminPageForm):
    ...
    def _get_assimilated_form(self, assimilated_class):
        ...
        return metaclass(class_name, (assimilated_class,), form_class_attrs)
...
class FormDefender(PageBase):
    ...
    def __new__(mcs, name, bases, attrs):
        new_attrs = dict(
            (k if k != "base_form_class" else "_" + k, v) for k, v in attrs.items()
        )
    ...
    def base_form_class(cls, form_class):
        ...
        if isinstance(form_class, type) and issubclass(form_class, my_form_class):
        ...
        if getattr(settings, "JOYOUS_DEFEND_FORMS", False):
            if issubclass(my_form_class, BorgPageForm):
                ...
                warning = FormCannotAssimilateWarning(
                    "{} cannot assimilate {}".format(
                        _getName(my_form_class), _getName(form_class)
                    )
                )
            ...
            warning = FormClassOverwriteWarning(
                ...
                "consider enabling JOYOUS_DEFEND_FORMS".format(
                    _getName(my_form_class), _getName(form_class)
                )
            )
            warnings.warn(warning, stacklevel=2)
