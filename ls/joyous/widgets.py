...
class RecurrenceWidget(MultiWidget):
    ...
    def decompress(self, value):
        ...
        if isinstance(value, Recurrence):
            if value.freq == WEEKLY:
                ...
                if value.byweekday:
                    if len(value.byweekday) == 7 and all(
                        not day.n for day in value.byweekday
                    ):
    ...
    def value_from_datadict(self, data, files, name):
        ...
        try:
            ...
            if freq == WEEKLY:
                ...
                if dayChoices[1] is not None and ordChoices[1] is not None:
                    wdayChoices.append(Weekday(dayChoices[1], ordChoices[1]))
                if dayChoices[2] is not None and ordChoices[2] is not None:
    ...
    def media(self):
        ...
        media += Media(
            css={"all": [static("joyous/css/recurrence_admin.css")]},
...
class ExceptionDateInput(AdminDateInput):
    ...
    def valid_dates(self):
        ...
        if self.overrides_repeat:
            ...
            valid_dates = [
                "{:%Y%m%d}".format(occurence)
                for occurence in self.overrides_repeat.between(past, future, inc=True)
            ]
        return valid_dates
