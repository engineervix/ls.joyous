...
class VResults:
    ...
    def __eq__(self, other):
        return (
            self.success == other.success
            and self.fail == other.fail
            and self.error == other.error
        )

    def __repr__(self):
        return "Success={}, Fail={}, Error={}".format(
...
class ICalHandler:
    ...
    def load(self, page, request, upload, **kwargs):
        ...
        if results.fail:
            messages.error(
                request, "Could not load {} iCal events".format(results.fail)
            )
...
class VCalendar(Calendar, VComponentMixin):
    ...
    def _updateEventPage(self, request, vevent, event):
        ...
        vchildren += [
            CancellationVEvent.fromExDate(vevent, exDate) for exDate in vevent.exDates
        ]
        for vchild in vchildren:
            try:
                exception = vchild.Page.objects.child_of(event).get(
                    except_date=vchild["RECURRENCE-ID"].date()
                )
            except ObjectDoesNotExist:
...
def _saveRevision(request, page):
    # Patch Django's datetime field to handle non-string values properly
    ...
    try:
        ...
        DateTimeField.to_python = original_to_python
...
class vDt(vDDDTypes):
    ...
    def __init__(self, value=None, *, inclusive=False):
        ...
        if value is not None:
            if hasattr(value, "dt"):
                value = value.dt
            if type(value) is dt.date and inclusive:
    ...
    def date(self, inclusive=False):
        if type(self.dt) is dt.datetime:
            return self.dt.date()
        elif type(self.dt) is dt.date:
            if inclusive:
...
class VEventFactory:
    ...
    def makeFromProps(self, props, parent):
        ...
        if type(dtstart.dt) is not type(dtend.dt):
        ...
        if recurrenceId is not None:
            ...
            if (
                parent
                and recurrenceId == dtstart
                and parent.numDays == numDays
                and parent["DTEND"].time() == dtend.time()
                and (
                    parent["SUMMARY"] != props["SUMMARY"]
                    or parent["DESCRIPTION"] != props["DESCRIPTION"] != ""
                )
            ):
...
class VEvent(Event, VComponentMixin):
    ...
    def exDates(self):
        ...
        retval = {
            exDate
            for vddd in exDates
            for exDate in vddd.dts
            if type(exDate.dt) is dtType
        }
        return list(retval)
...
class RecurringVEvent(VEvent):
    ...
    def __getExceptions(cls, page):
        ...
        for cancellation in CancellationPage.events.child_of(page):
            excludeDt = getAwareDatetime(
                cancellation.except_date,
        ...
        for shutdown in ExtCancellationPage.events.child_of(page):
            ...
            for shutDate in shutdown._getMyDates(toDate=dt.date(MAX_YEAR, 1, 1)):
                excludeDt = getAwareDatetime(
                    shutDate, shutdown.time_from, shutdown.tz, dt.time.min
                )
                excludes.add(excludeDt)
                # NB any cancellation title or details are going to be lost
        ...
        closedHols = (
            ClosedForHolidaysPage.events.hols(page.holidays).child_of(page).first()
        )
        if closedHols is not None:
            for closedDate in closedHols._getMyDates():
                if closedDate.year > MAX_YEAR:
                    break
                excludeDt = getAwareDatetime(
                    closedDate, closedHols.time_from, closedHols.tz, dt.time.min
                )
                excludes.add(excludeDt)
                # NB any cancellation title or details are going to be lost
        ...
        for info in ExtraInfoPage.events.child_of(page):
            excludeDt = getAwareDatetime(
                info.except_date, info.time_from, info.tz, dt.time.min
            )
    ...
    def toPage(self, page):
        ...
        page.details = self._getDesc()
        page.location = str(self.get("LOCATION", ""))
        page.repeat = Recurrence(rrule.to_ical().decode(), dtstart=dtstart.date())
...
class ExceptionVEvent(VEvent):
    ...
    def fromPage(cls, page):
        ...
        exceptDt = getAwareDatetime(
            page.except_date, page.overrides.time_from, page.tz, dt.time.min
        )
        dtstart = getAwareDatetime(
            page.except_date, page.time_from, page.tz, dt.time.min
        )
        dtend = getAwareDatetime(
            page.except_date + daysDelta, page.time_to, page.tz, dt.time.max
        )
...
class PostponementVEvent(ExceptionVEvent):
    ...
    def fromPage(cls, page):
        ...
        dtstart = getAwareDatetime(page.date, page.time_from, page.tz, dt.time.min)
        dtend = getAwareDatetime(
            page.date + daysDelta, page.time_to, page.tz, dt.time.max
        )
