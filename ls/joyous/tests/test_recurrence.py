...
class TestRecurrence(TestCase):
    ...
    def testInitRecurrence(self):
        rr1 = Recurrence(
            dtstart=dt.date(2009, 1, 1), freq=WEEKLY, byweekday=[MO, TU, WE, TH, FR]
        )
        rr2 = Recurrence(rr1)
    ...
    def testEq(self):
        rr1 = rrule(
            dtstart=dt.datetime(2009, 1, 1, 0, 0, 1),
            freq=WEEKLY,
    ...
    def testRoundtrip(self):
        ...
        rrStr = (
            "DTSTART:20141001\n"
            "RRULE:FREQ=MONTHLY;WKST=SU;UNTIL=20141001;BYMONTHDAY=1,-1"
        )  # first&last
        self.assertEqual(repr(Recurrence(rrStr)), rrStr)
    ...
    def testGetRrule(self):
        ...
        self.assertEqual(rr._getRrule(), "FREQ=DAILY;INTERVAL=2;WKST=SU;UNTIL=20110111")
        with self.assertRaises(TypeError):
            rr._getRrule(untilDt=dt.datetime(2011, 1, 11))
        self.assertEqual(
            rr._getRrule(
                untilDt=dt.datetime(2011, 1, 11, 23, 59, 59, tzinfo=dt.timezone.utc)
            ),
...
class TestGetWhen(TestCase):
    ...
    def testEvery2Days(self):
        rr = Recurrence(dtstart=dt.date(2009, 1, 1), interval=2, freq=DAILY)
        self.assertEqual(rr._getWhen(3), "Every 2 days")

    def testMonEveryFortnight(self):
        rr = Recurrence(
            dtstart=dt.date(2009, 1, 1), interval=2, freq=WEEKLY, byweekday=MO
        )
        self.assertEqual(rr._getWhen(0), "Fortnightly on Mondays")

    def testMonEvery6Weeks(self):
        rr = Recurrence(
            dtstart=dt.date(2009, 1, 1), interval=6, freq=WEEKLY, byweekday=MO
        )
    ...
    def testLastSatSeptEvery2Years(self):
        ...
        self.assertEqual(
            rr._getWhen(0, numDays=5),
            "The last Saturday of September, every 2 years for 5 days",
        )

    def test1st(self):
        rr = Recurrence(dtstart=dt.date(2009, 1, 1), freq=MONTHLY, bymonthday=1)
        self.assertEqual(rr._getWhen(0), "The first day of the month")

    def test22ndOffsetNeg1(self):
        rr = Recurrence(
            dtstart=dt.date(2009, 1, 1), freq=YEARLY, bymonthday=22, bymonth=5
        )
        self.assertEqual(rr._getWhen(-1), "The 21st day of May")

    def test30thOffset1(self):
        rr = Recurrence(dtstart=dt.date(2009, 1, 1), freq=MONTHLY, bymonthday=30)
        self.assertEqual(rr._getWhen(1), "The day after the 30th day of the month")

    def testMonWedFriOffset1(self):
        rr = Recurrence(
            dtstart=dt.date(2009, 1, 1), freq=WEEKLY, count=9, byweekday=[MO, WE, FR]
        )
        self.assertEqual(rr._getWhen(1), "Tuesdays, Thursdays and Saturdays")

    def test2ndAnd4thFriOffsetNeg1(self):
        rr = Recurrence(
            dtstart=dt.date(2009, 1, 1), freq=MONTHLY, byweekday=[FR(2), FR(4)]
        )
        self.assertEqual(
            rr._getWhen(-1),
            "The Thursday before the second Friday and "
    ...
    def test1stOffsetNeg1(self):
        ...
        self.assertEqual(
            rr._getWhen(-1), "The last day of the month (until 30 April 2010)"
        )

    def test3rdOffset2(self):
        rr = Recurrence(dtstart=dt.date(2009, 1, 1), freq=MONTHLY, bymonthday=3)
        self.assertEqual(rr._getWhen(2), "The fifth day of the month")

    def test1stJanAprMayOffsetNeg1(self):
        rr = Recurrence(
            dtstart=dt.date(2009, 1, 1), freq=YEARLY, bymonthday=1, bymonth=[1, 4, 5]
        )
    ...
    def testLastJulAugSepDecOffset1(self):
        ...
        self.assertEqual(
            rr._getWhen(1), "The first day of August, September, October and January"
        )

    def test1stAndLast(self):
        rr = Recurrence(dtstart=dt.date(2009, 1, 1), freq=MONTHLY, bymonthday=[1, -1])
