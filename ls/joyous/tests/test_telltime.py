...
class TestLocalTimes(TestCase):
    ...
    def testGetLocalDatetime(self):
        self.assertEqual(
            getLocalDatetime(dt.date(2019, 1, 1), dt.time(1)), datetimetz(2019, 1, 1, 1)
        )
        when = getLocalDatetime(
            dt.date(2003, 9, 2), dt.time(10, 45, 1), pytz.timezone("Asia/Tokyo")
        )
    ...
    def testGetLocalDateAndTime(self):
        ...
        date, time = getLocalDateAndTime(
            dt.date(2011, 4, 28), None, pytz.timezone("Asia/Yerevan")
        )
        self.assertEqual(date, dt.date(2011, 4, 29))
        self.assertEqual(time, None)

    def testGetLocalDate(self):
        date = getLocalDate(dt.date(1993, 8, 8), None, pytz.timezone("Europe/London"))
        self.assertEqual(date, dt.date(1993, 8, 9))

    def testGetLocalTime(self):
        time = getLocalTime(
            dt.date(2018, 5, 8), dt.time(22, 44), pytz.timezone("Pacific/Auckland")
        )
        self.assertEqual(time, dt.time(19, 44))

    def testGetLocalTimeAtDate(self):
        time = getLocalTimeAtDate(
            dt.date(2018, 5, 8), dt.time(22, 44), pytz.timezone("Pacific/Auckland")
        )
        self.assertEqual(time, dt.time(19, 44))
    ...
    def testGetLocalTimeAtDateOffset1(self):
        time = getLocalTimeAtDate(
            dt.date(2019, 6, 28), dt.time(8, 10), pytz.timezone("America/Los_Angeles")
        )
    ...
    def testGetLocalTimeAtDateOffset2(self):
        time = getLocalTimeAtDate(
            dt.date(2019, 1, 1), dt.time(23, 30), pytz.timezone("Pacific/Pago_Pago")
        )
...
class TestFormats(TestCase):
    def testTimeFormat(self):
        ...
        self.assertEqual(
            timeFormat(dt.time(20), dt.time(2), "at ", "->"), "at 8pm -> 2am"
        )
        ...
        with override_settings(JOYOUS_TIME_FORMAT="Hi"):
            self.assertEqual(timeFormat(dt.time(8), dt.time(11)), "0800 to 1100")

    def testDateFormat(self):
        self.assertEqual(dateFormat(None), "")
        self.assertEqual(
            dateFormat(dt.date(2017, 2, 16)), "Thursday 16th of February 2017"
        )
        today = dt.date.today()
        ...
        with override_settings(JOYOUS_DATE_FORMAT=None):
            self.assertEqual(dateFormat(dt.date(2017, 2, 16)), "Feb. 16, 2017")
    ...
    def testDateShortFormat(self):
        ...
        with override_settings(JOYOUS_DATE_SHORT_FORMAT=None):
            self.assertEqual(dateShortFormat(dt.date(2017, 2, 16)), "02/16/2017")
