...
class TestRecurrenceWidget(TestCase):
    ...
    def testDecompressDayOfMonth(self):
        rr = Recurrence(dtstart=dt.date(2019, 1, 1), freq=MONTHLY, bymonthday=1)
        widget = RecurrenceWidget()
        self.assertEqual(
            widget.decompress(rr),
            [
    ...
    def testWeekdaysValue(self):
        ...
        self.assertEqual(str(widget.value_from_datadict(data, {}, "repeat")), str(rr))
    ...
    def testSameTuesdayValue(self):
        ...
        rr = Recurrence(dtstart=dt.date(2010, 1, 12), freq=MONTHLY, byweekday=[TU(2)])
        self.assertEqual(widget.value_from_datadict(data, {}, "repeat"), rr)
    ...
    def test135WednesdaysValue(self):
        ...
        rr = Recurrence(
            dtstart=dt.date(2010, 1, 12), freq=MONTHLY, byweekday=[WE(1), WE(3), WE(5)]
        )
        self.assertEqual(widget.value_from_datadict(data, {}, "repeat"), rr)

    def testGetContext(self):
        rr = Recurrence(
            dtstart=dt.date(2014, 12, 1), freq=MONTHLY, byweekday=[MO(1), MO(3), MO(5)]
        )
        attrs = {"id": "id_repeat"}
        ...
        self.assertEqual(ctx["template_name"], "joyous/widgets/recurrence_widget.html")
    ...
    def testMedia(self):
        widget = RecurrenceWidget()
        self.assertIn(
            "/static/joyous/css/recurrence_admin.css", widget.media._css["all"]
        )
...
class TestExceptionDateInput(TestCase):
    ...
    def testRenderNone(self):
        ...
        self.assertHTMLEqual(
            lines[0],
            """
<input type="text" name="xdate" id="id_xdate" autocomplete="{0.newDate}">""".format(
                self
            ),
        )
    ...
    def testValidDates(self):
        widget = ExceptionDateInput()
        widget.overrides_repeat = Recurrence(
            dtstart=dt.date(2009, 1, 1), freq=MONTHLY, byweekday=MO(1)
        )
        self.assertEqual(
            widget.valid_dates(),
            [
    ...
    def testMedia(self):
        widget = ExceptionDateInput()
        self.assertEqual(
            widget.media._css, {"all": ["/static/joyous/css/recurrence_admin.css"]}
        )
