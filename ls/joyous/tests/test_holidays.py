...
class TestHolidays(TestCase):
    ...
    def testMultiHolidays(self):
        ...
        self.assertEqual(
            hols.get(dt.date(1999, 1, 1)), "Gliffy, Whatnot, New Year's Day"
        )
    ...
    def testWorkalendarNames(self):
        class Woral:
            get_calendar_holidays = Mock(
                return_value=[(dt.date(1999, 4, 30), "JOY JOY")]
            )
    ...
    def testAdd(self):
        ...
        self.assertEqual(tasHols.get(dt.date(2020, 10, 20)), "Kangaroo Day, Kiwi Day")
...
class TestParser(TestCase):
    ...
    def testCountriesNE(self):
        ...
        self.assertEqual(
            hols.get(dt.date(2019, 3, 1)), "Jahrestag der Ausrufung der Republik"
        )
        self.assertEqual(hols.get(dt.date(2019, 4, 26)), "Arbor Day")

    def testNorthIsland(self):
        hols = parseHolidays("NZ[NTL,AUK,HKB,TKI,WGN]")
        self.assertEqual(hols.get(dt.date(2020, 1, 20)), "Wellington Anniversary Day")
        self.assertEqual(hols.get(dt.date(2020, 1, 27)), "Auckland Anniversary Day")
        self.assertEqual(hols.get(dt.date(2020, 3, 9)), "Taranaki Anniversary Day")
        self.assertEqual(hols.get(dt.date(2020, 10, 23)), "Hawke's Bay Anniversary Day")

    def testNelson(self):
        hols = parseHolidays("NZ[Nelson]")
        self.assertEqual(hols.get(dt.date(2020, 2, 3)), "Nelson Anniversary Day")
